...
$('#form').onsubmit = e => {
  ...
  // Los ítems ya están actualizados en state.invoices[state.edit].items
  ...
  const inv = {
    ...
    client, date, concept, price, items, subtotal, iva, total,
    paid: isPaid,
...
window.printInvoice = i => {
  ...
  win.document.write(`
    ...
    <div class="container">
      ...
      <div class="info-grid">
        <div class="info-box"><strong>Cliente:</strong><br>${inv.client}</div>
...
const Config = {
  ...
  saveAndStart: () => {
    ...
    state.config = { name, nif, address: $('#cfgAddress').value.trim(), phone: $('#cfgPhone').value.trim(), iban: $('#cfgIban').value.trim() };
    saveCfg();
...
const Export = {
  backup: () => {
    ...
    const a = document.createElement('a'); a.href = data; a.download = 'facturas.json'; a.click();
  },
  import: () => {
